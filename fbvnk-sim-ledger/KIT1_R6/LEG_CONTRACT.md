...
| Case | What accrues |
|---|---|
| Normal | this record's `tachStop` less the **previous record's `tachStop`**, to 0.01 h |
| The first record the book sees | its own `tachStop − tachStart`; the cursor is picked up there |
| `tachStart` below the cursor | a reload or another machine. Flagged, **nothing negative posted**, this leg accrues its own span, the cursor resumes from the new reading |
...
## What the review page shows, and why it is not the carnet's format

Claus, 16 Sep 21:0x: *"update the cost ledger with what the lates layout of the lookbook looks
like"*. The review-and-sign page now reproduces the **V275 iPad logbook panel** line for line, from
the installed `HANGAR.js` `lbLastText()` strings rather than from a description:

```
Leg 12: 08:20Z - 09:41Z (engine start - shutdown)
Airborne 1:03, block 1:21
Fuel 220 L -> 194 L (26.4 L)
Tacho 1228.32 -> 1229.34
Take-off 08:32Z, 1 landing, last 09:35Z at -180 fpm
Position dep 38.726 -9.355, arr 40.156 -8.469
Zulu day 259
```

Times `HH:MMZ`, durations `h:mm` with the hour unpadded, REM in whole litres with the burn to 0.1 L
in brackets, tacho to two decimals, positions to three, touchdown in whole fpm. The refuel, air
start and engine-stopped variants use the installed strings too.

**Three deliberate differences from the rest of the book, each stated on the page itself:**

- **Always Zulu.** The carnet has a UTC/local control and the iPad has none. A line meant to match
  must not move when that control does.
- **The tacho takes a dot**, because the iPad prints `toFixed(2)`. The carnet keeps the comma Claus
  writes by hand on page 86. Same number, two hands.
- **The touchdown is shown negative-down — the sim's own sign — because that is what the iPad
  prints.** `touchdownFpm` in the record stays **positive-down** per this contract, and that is what
  the book bills and checks hard landings on. Both signs are named under the panel. Two signs for
  one number is exactly what gets read wrong six weeks later, so neither is left implicit.
