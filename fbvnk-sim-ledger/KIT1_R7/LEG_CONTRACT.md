...
> resolving them — the radius, the coordinate set, the `????` code, the aerodrome-positions panel,
...
| `from`, `to` | ICAO | **V277**: the airport the sim reports at engine start and at shutdown | Taken as sent. Missing → `----`, priced at the default field, and the pilot types the code on the review page. **Revision 7 dropped the four position fields.** |
